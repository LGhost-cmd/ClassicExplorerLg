...
# =============================================================================
# Media A/V v5.5 - RichEdit redraw/layout stabilization
# - never touch the inline audio object while an older-history batch is being
#   inserted into the RichEdit control
# - verify the target character is really an OLE object before replacing it
# - preserve the exact chat scroll position while repainting the audio player
# - force an erased repaint after a history transaction to prevent ghost text
# =============================================================================

# -----------------------------------------------------------------------------
# telegacy.h: cross-file guard used by response.cpp while it mutates chat.
# -----------------------------------------------------------------------------
s = read(h)

if "media_chat_layout_transaction" not in s:
    anchor = "extern bool media_archive_preserve_on_chat_clear;"

    if anchor not in s:
        raise SystemExit(
            "Could not locate media_archive_preserve_on_chat_clear declaration."
        )

    s = s.replace(
        anchor,
        anchor + "\nextern bool media_chat_layout_transaction;",
        1,
    )
...
# -----------------------------------------------------------------------------
# telegacy.cpp: make audio bitmap replacement non-destructive and repaint-safe.
# -----------------------------------------------------------------------------
s = read(t)

if "media_inline_audio_redraw_guard_v55" not in s:
    global_anchor = "bool media_archive_preserve_on_chat_clear = false;"

    if global_anchor not in s:
        raise SystemExit(
            "Could not locate Media chat-clear state global."
        )
    ...
    s = s.replace(
        ...
        + "\nbool media_chat_layout_transaction = false;",
        1,
    )
    ...
    # Never perform EM_SETSEL/EM_REPLACESEL while response.cpp is prepending a
    # getHistory page. The Document character offsets are being shifted during
    # that transaction and touching them mid-layout can repaint stale pixels or
    # (in the worst case) target normal text.
    validation_anchor = r'''    if (
        !chat ||
        document_index < 0 ||
        document_index >=
            (int)documents.size()
...
'''
    ...
    validation_new = validation_anchor + r'''
    if (media_chat_layout_transaction)
        return false;
'''

    if validation_anchor not in fn:
        raise SystemExit(
            "Could not locate inline audio bitmap validation block."
        )
    ...
    # Sanity-check the RichEdit character before removing it. Embedded OLE
    # objects are represented as U+FFFC. If bookkeeping ever lags behind a
    # prepend/edit, refusing to update is much safer than deleting chat text.
    object_range_anchor = r'''    if (
        object_pos < 0 ||
        object_pos >= text_length
    ) {
        return false;
...
'''
    ...
    object_range_new = object_range_anchor + r'''

    wchar_t object_marker[2] = {0, 0};
    TEXTRANGEW object_range = {0};
    object_range.chrg.cpMin = object_pos;
    object_range.chrg.cpMax = object_pos + 1;
    object_range.lpstrText = object_marker;
    ...
    if (object_marker[0] != 0xFFFC) {
        diag_log(
            "inline audio visual skip: expected OLE marker at char=%d got=0x%04X",
            object_pos,
            (unsigned int)object_marker[0]
        );
        return false;
    }
'''

    if object_range_anchor not in fn:
        raise SystemExit(
            "Could not locate inline audio object range check."
        )
    ...
    # Save the viewport. EM_SETSEL on an off-screen OLE object can otherwise
    # make RichEdit scroll internally before the previous selection is restored.
    selection_anchor = r'''    CHARRANGE saved = {0};
    SendMessageW(
        chat,
        EM_EXGETSEL,
        0,
...
'''
    ...
    selection_new = selection_anchor + r'''

    POINT saved_scroll = {0, 0};
    SendMessageW(
        chat,
        EM_GETSCROLLPOS,
        0,
...
'''
    ...
    if selection_anchor not in fn:
        raise SystemExit(
            "Could not locate inline audio selection save block."
        )
    ...
    # Restore the viewport before allowing redraw again.
    restore_anchor = r'''    SendMessageW(
        chat,
        EM_EXSETSEL,
        0,
        (LPARAM)&saved
...
'''
    ...
    if restore_anchor not in fn:
        raise SystemExit(
            "Could not locate inline audio redraw restore block."
        )
    ...
    # Erase the old player pixels before repainting the new bitmap. Merely
    # invalidating without background erase is what can leave stale text/OLE
    # fragments after a scroll or large history prepend.
    invalidate_anchor = r'''    InvalidateRect(
        chat,
        &player_rect,
        FALSE
    );
'''
    ...
    invalidate_new = r'''    RECT redraw_rect = player_rect;
    InflateRect(
        &redraw_rect,
        3,
        3
...
'''
    ...
    if invalidate_anchor not in fn:
        raise SystemExit(
            "Could not locate inline audio player invalidation block."
        )
...
# -----------------------------------------------------------------------------
# response.cpp: guard the getHistory prepend as an explicit layout transaction
# and perform a true erased redraw when it completes.
# -----------------------------------------------------------------------------
s = read(r)

if "history_layout_guard_v55" not in s:
    history_if = r'''\t\tif (history_keep_view) {
\t\t\thistory_scrollbar_before.cbSize =
'''.replace('\\t','\t')

    history_if_new = r'''\t\tif (history_keep_view) {
\t\t\t// history_layout_guard_v55
\t\t\tmedia_chat_layout_transaction = true;

\t\t\thistory_scrollbar_before.cbSize =
'''.replace('\\t','\t')

    if history_if not in s:
        raise SystemExit(
            "Could not locate v5.4 history viewport transaction start."
        )
...
'''.replace('\\t','\t')
    ...
    redraw_new = r'''\t\t\t// All message/document character positions are stable again.
...
'''.replace('\\t','\t')
    ...
    if redraw_anchor not in s:
        raise SystemExit(
            "Could not locate v5.4 final history redraw."
        )
...
for path, tokens in checks.items():
    ...
    for token in tokens:
        if token not in data:
            raise SystemExit(
                f"Media v5.5 verification failed in {path.name}: {token}"
            )

print(
    "Applied Media A/V v5.5: safe inline OLE replacement, preserved viewport, "
    "history-layout guard, and full erased redraw after prepend."
)
